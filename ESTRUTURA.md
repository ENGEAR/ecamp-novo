...
- **QAR Externo — Particulados (PTS/PM10/PM2,5) — PRONTO.** Romaneio próprio + tela por ponto: identificação (nome + **característica do ambiente**, ex.: "fluxo intenso de veículos, próximo estrada não pavimentada"; Tipo de equip. vem dos equipamentos selecionados) · **calibração** em 6 passos (aquecimento · zerar manômetro · teste de vazamento 800/400 mm com **cronômetro** · porta filtro · condições ambientais (temperatura/pressão/umidade + vento [Fraco/Médio/Forte] + "como está o tempo") · grade de 5 cartas 18/13/10/09/08 + leitura com filtro + "calibração aprovada"/validade) · **coletas** (quantidade por ponto; cada uma com código do filtro + dados iniciais e finais, incluindo vento [Fraco/Médio/Forte] e "como está o tempo" nos dois) · hora final. Equipamentos: amostradores AGV (cadastrados à mão; não estão no F021). Foto obrigatória por ponto. Os subtipos **Gases/Trigás** e **Poeira Sedimentável** do QAR Externo ainda entram como "em construção".
...
- **Outro (genérico) — PRONTO.** Geral: tipo de monitoramento (texto), objetivo, qtde de pontos. Por ponto: nome, GPS, hora inicial, medição principal + unidade, foto da tela + foto do ponto, clima, observações. Equipamentos = cadastro manual.
- **QAR Interno (MQAI) — PRONTO.** Navegação **aninhada**: qtde de ambientes → (por ambiente) nome, área, "calcular pontos" (tabela por área **+ 1 ponto externo de referência**) → (por ponto) identificação, posicionamento (8 checks), valor da vazão, coleta de fungos (checklist + horário inicial/final + volume amostrado), medições (CO₂/temp/UR/vel ar/PM2,5/PM10/partículas), conformidade, coleta de filtro (nº do filtro + checklist + horário inicial/final + volume coletado), transporte, **3 fotos** (ponto/tela/ambiente), hora final, obs. Só o essencial trava o salvar (checklists de fungos/filtro/transporte = orientação; os campos de horário/volume não travam). Equipamentos: F021 "Qualidade do ar interno" (bomba, medidor CO₂, tripés).
...
- **Escopo de OS por usuário** (`js/os.js` → `carregarEscopo`/`dentroEscopo`): na tela de Serviços, quem tem papel `logistica` (ou `admin`) vê **todas** as OS; os demais só as OS em que estão escalados na Agenda. Vínculo: e-mail do login → `tecnicos.email` → nome do técnico → `agendamentos.tecnicos[]` → `ordem_servico_id`, que casa com `os.osId` (UUID×UUID). É filtro de tela (app-only, não barreira de servidor); cacheado por e-mail em `os:escopo` para valer offline. Exige o técnico ter e-mail preenchido no cadastro do SGP.
- **"Em andamento por quem"** (`js/os.js` → `carregarAndamentoPor`/`andamentoPor`): o cartão de OS mostra uma tag roxa "⏳ Em andamento · Fulano" com o(s) primeiro(s) nome(s) de quem está preenchendo. Lê os `monitoramentos` com status `rascunho` direto do Supabase (RLS libera leitura a qualquer logado), casando `monitoramentos.tecnico` (= nome do usuário que preencheu) por nº da OS. App-only, cacheado em `os:andamentoPor`; funciona entre aparelhos da equipe.
